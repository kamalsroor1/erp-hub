...
<template>
    <teleport to="body">
        <div v-if="show" class="fixed inset-0 z-50 overflow-hidden font-tajawal" dir="rtl">
            ...
            <div
                @click="emit('close')"
                class="fixed inset-0 bg-slate-950/80 backdrop-blur-xs transition-opacity duration-300"
            ></div>

            <!-- Slide-Over Drawer Container -->
            <div class="fixed inset-y-0 left-0 max-w-full flex pl-0 sm:pl-10">
                <div
                    class="w-screen max-w-md bg-slate-900 border-r border-slate-800 shadow-2xl flex flex-col justify-between transform transition-transform duration-300 ease-in-out animate-in slide-in-from-left"
                >
                    <!-- Drawer Header -->

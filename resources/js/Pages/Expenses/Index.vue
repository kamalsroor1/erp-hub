...
const costCenterOptions = computed(() => {
    return [
        { id: 'all', name: 'كافة مراكز التكلفة' },
        ...Object.entries(props.cost_centers).map(([k, v]) => ({ id: k, name: v }))
    ];
});

const paymentMethodOptions = [
    { id: 'all', name: 'كافة طرق الدفع' },
    { id: 'cash', name: 'نقداً كاش 💵' },
    { id: 'instapay', name: 'انستاباي InstaPay ⚡' },
    { id: 'e_wallet', name: 'محفظة إلكترونية 📱' },
    { id: 'visa', name: 'فيزا وبطاقة بنكية 💳' },
...
const deleteExpense = (e) => {
    if (confirm(`هل أنت متأكد من حذف المصروف (${e.title})؟`)) {
...
<template>
    <Head title="سجل المصروفات التشغيلية والنثريات" />
    ...
    <AppLayout>
        <div class="space-y-6 font-tajawal">
                ...
                <div class="space-y-1">
                    <div class="flex items-center gap-2">
                        ...
                        <h1 class="text-xl sm:text-2xl font-black text-white">
                            سجل المصروفات التشغيلية والنثريات ومراكز التكلفة
                        </h1>
                    </div>
                    <p class="text-xs text-slate-400 font-bold">
                        إثبات نفقات التشغيل، الرواتب، الإيجارات، وتأثيرها على حساب الأرباح والخزينة
                    </p>
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div class="bg-slate-900 border border-slate-800 rounded-3xl p-5 shadow-sm space-y-2">
                    <span class="text-xs text-slate-400 font-bold">إجمالي مصروفات هذا الشهر</span>
                    <div class="text-2xl font-black font-mono text-rose-400">
                        {{ formatMoney(metrics.total_month) }} <span class="text-xs text-white">ج.م</span>
                    </div>
                </div>

                <div class="bg-slate-900 border border-slate-800 rounded-3xl p-5 shadow-sm space-y-2">
                    <span class="text-xs text-slate-400 font-bold">مصروفات مسحوبة من الدرج كاش</span>
                    <div class="text-2xl font-black font-mono text-amber-400">
                        {{ formatMoney(metrics.total_cash) }} <span class="text-xs text-white">ج.م</span>
                    </div>
                </div>

                <div class="bg-slate-900 border border-slate-800 rounded-3xl p-5 shadow-sm space-y-2">
                    <span class="text-xs text-slate-400 font-bold">إجمالي المصروفات وفق التصفية الحالية</span>
                    <div class="text-2xl font-black font-mono text-white">
                        {{ formatMoney(metrics.total_filtered) }} <span class="text-xs text-amber-400">ج.م</span>
                    </div>
                </div>
            </div>
                ...
                <div class="flex flex-col md:flex-row items-center justify-between gap-3">
                    <div class="w-full md:w-96 relative">
                        <input
                            v-model="search"
                            type="text"
                            placeholder="... بحث برقم المصروف أو البيان أو التصنيف"
            ...
            <div class="bg-slate-900 border border-slate-800 rounded-3xl p-5 shadow-sm space-y-4 overflow-hidden">
                <div class="overflow-x-auto">
                            ...
                            <tr v-for="e in expenses.data" :key="e.id" class="hover:bg-slate-800/30 transition">
                                ...
                                <td class="py-3.5 font-tajawal">
                                    <span class="px-2 py-0.5 rounded-full text-[10px] font-bold bg-slate-800 text-slate-300">
                                        {{ e.payment_method === 'cash' ? 'كاش نقداً 💵' : e.payment_method }}
                                    </span>
                                </td>
                    ...
                    <div v-if="!expenses.data || expenses.data.length === 0" class="py-16 text-center space-y-2">
                        ...
                        <p class="text-xs font-bold text-slate-400 font-tajawal">لا توجد مصروفات مسجلة مطابقة للبحث</p>
                    </div>
                </div>
                ...
                <div v-if="expenses.links && expenses.links.length > 3" class="pt-4 border-t border-slate-800/80 flex items-center justify-between font-sans">
                    <span class="text-xs text-slate-400 font-tajawal">
                        عرض {{ expenses.from || 0 }} إلى {{ expenses.to || 0 }} من إجمالي {{ expenses.total || 0 }} مصروف
                    </span>
        ...
        >
            <div class="space-y-5">
                ...
                <div class="space-y-1.5">
                    <label class="text-xs font-black text-slate-300">🏢 مركز التكلفة</label>
                    <SearchableSelect
                        v-model="costCenter"
                ...
                <div class="space-y-1.5">
                    <label class="text-xs font-black text-slate-300">💳 طريقة الدفع</label>
                    <SearchableSelect
            ...
            <div @click.stop class="w-full max-w-lg bg-slate-900 border border-slate-800 rounded-3xl p-6 shadow-2xl space-y-4">
                <div class="flex items-center justify-between border-b border-slate-800 pb-3">
                    <h3 class="font-black text-base text-white">
                        {{ editingExpense ? 'تعديل بيانات المصروف' : 'تسجيل وقيد مصروف تشغيلي جديد 💸' }}
                    </h3>
                ...
                <form @submit.prevent="saveExpense" class="space-y-4">
                    <div class="space-y-1.5">
                        <label class="text-xs font-bold text-slate-300">بيان وعنوان المصروف *</label>
                        <input
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        <div class="space-y-1.5">
                            <label class="text-xs font-bold text-slate-300">تاريخ المصروف *</label>
                            <DatePicker v-model="expenseForm.expense_date" placeholder="تاريخ المصروف..." />
                        </div>
                    </div>

                    <div class="grid grid-cols-2 gap-3">
                        <div class="space-y-1.5">
                            <label class="text-xs font-bold text-slate-300">مركز التكلفة *</label>
                            <select
                        ...
                        <div class="space-y-1.5">
                            <label class="text-xs font-bold text-slate-300">طريقة الدفع *</label>
                            <select
                    ...
                    <div class="space-y-1.5">
                        <label class="text-xs font-bold text-slate-300">التصنيف السريع</label>
                    ...
                    <div class="flex items-center justify-end gap-2 pt-3 border-t border-slate-800">
                        ...
                        >
                            {{ expenseForm.processing ? 'جاري القيد...' : 'حفظ المصروف' }}
                        </button>
                    </div>
                </form>
            </div>

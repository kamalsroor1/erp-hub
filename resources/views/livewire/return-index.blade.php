<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row items-center justify-between gap-4 bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
        ...
        <div class="flex items-center gap-2 text-xs">
            <span class="text-slate-500 dark:text-slate-400">نوع المرتجع:</span>
            <button wire:click="$set('type', 'all')" class="px-3 py-1.5 rounded-lg font-bold border transition-colors cursor-pointer {{ $type === 'all' ? 'bg-slate-200 dark:bg-slate-800 border-slate-300 dark:border-slate-600 text-slate-900 dark:text-white' : 'border-transparent text-slate-500 dark:text-slate-400' }}">الكل</button>
            <button wire:click="$set('type', 'sales_return')" class="px-3 py-1.5 rounded-lg font-bold border transition-colors cursor-pointer {{ $type === 'sales_return' ? 'bg-emerald-500/20 border-emerald-500/40 text-emerald-700 dark:text-emerald-400' : 'border-transparent text-slate-500 dark:text-slate-400' }}">مرتجع مبيعات (من عميل)</button>
            <button wire:click="$set('type', 'purchase_return')" class="px-3 py-1.5 rounded-lg font-bold border transition-colors cursor-pointer {{ $type === 'purchase_return' ? 'bg-amber-500/20 border-amber-500/40 text-amber-700 dark:text-amber-400' : 'border-transparent text-slate-500 dark:text-slate-400' }}">مرتجع مشتريات (لمورد)</button>
        </div>
    </div>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-right text-xs">
                ...
                <tbody class="divide-y divide-slate-200 dark:divide-slate-800/60">
                    ...
                    @empty
                    <tr>
                        <td colspan="7" class="p-12 text-center text-slate-400">لا توجد مرتجعات مسجلة</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>

<div class="space-y-4">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white dark:bg-slate-900/60 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
        <div>
            ...
            <p class="text-xs text-slate-500 dark:text-slate-400">بيع دقيق بالوزن (إيداع بالكيلو والشكاير والبيع بربع وثمن ونصف كيلو وبالجرام) مع حساب الكسور لحظياً</p>
        </div>
        <div class="flex items-center gap-2">
            <span class="hidden sm:inline-flex text-[11px] sm:text-xs text-amber-700 dark:text-amber-400 font-bold bg-amber-500/10 border border-amber-500/20 px-3 py-1 rounded-xl">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-4">
            ...
            <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm">
                <div class="p-3 bg-slate-50 dark:bg-slate-950/60 border-b border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-1 text-xs font-bold text-slate-700 dark:text-slate-300">
                    ...
                    <span class="text-[11px] text-amber-600 dark:text-amber-400 font-normal">يمكنك إدخال الوزن بالكسور (مثال 0.250 كجم لربع كيلو أو 0.125 لثمن كيلو)</span>
                </div>
                            ...
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/20">
                                ...
                                <td class="p-3">
                                    <input 
                                        ...
                                        class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 rounded-lg px-2 py-1 text-center text-xs font-mono text-slate-900 dark:text-white focus:outline-none focus:border-emerald-500"
                                    >
                                </td>
                                <td class="p-3">
                                    <input 
        ...
        <div class="space-y-4">
            ...
            <div class="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-3 shadow-sm">
                ...
                <select wire:model.live="customer_id" class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 rounded-xl px-3 py-2.5 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-emerald-500">
                ...
                <div class="pt-2">
                    ...
                    <input 
                        ...
                        class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 rounded-xl px-3 py-2 text-xs font-mono font-bold text-slate-900 dark:text-white focus:outline-none focus:border-emerald-500"
                    >
                </div>
                @endif
            </div>
            ...
            <div class="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-4 shadow-sm">
                ...
                <div class="grid grid-cols-2 gap-2">
                    <div>
                        <label class="block text-[11px] font-bold text-slate-500 dark:text-slate-400 mb-1">نوع الخصم:</label>
                        <select wire:model.live="discount_type" class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 rounded-lg px-2 py-1.5 text-xs text-slate-900 dark:text-white">
                            <option value="fixed">مبلغ ثابت (ج.م)</option>
                            <option value="percentage">نسبة مئوية (%)</option>
                        </select>
                    </div>
                    <div>
                        <label class="block text-[11px] font-bold text-slate-500 dark:text-slate-400 mb-1">قيمة الخصم:</label>
                        <input type="number" step="0.01" min="0" wire:model.live.debounce.250ms="discount_value" class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 rounded-lg px-2 py-1.5 text-xs font-mono text-rose-600 dark:text-rose-400 text-center">
                    </div>
                </div>

                <!-- Totals Breakdown -->
                <div class="space-y-2 pt-2 text-xs border-t border-slate-200 dark:border-slate-800/80">
                    ...
                    <div class="flex justify-between font-bold {{ bccomp($remaining_amount, '0.000', 3) > 0 ? 'text-amber-600 dark:text-amber-400' : 'text-slate-500 dark:text-slate-400' }}">
                ...
                <div class="space-y-2 pt-4">
                    <button 
                        ...
                        class="w-full py-3 bg-gradient-to-r from-emerald-600 to-teal-500 hover:from-emerald-500 hover:to-teal-400 text-white font-bold rounded-xl shadow-lg shadow-emerald-600/30 flex items-center justify-center gap-2 transition-all active:scale-95 text-xs cursor-pointer"
                    >
                    ...
                    <button 
                        ...
                        class="w-full py-2.5 bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 text-xs font-bold rounded-xl border border-slate-300 dark:border-slate-700 flex items-center justify-center gap-2 transition-all cursor-pointer"
                    >

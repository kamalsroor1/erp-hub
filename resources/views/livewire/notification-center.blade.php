<div class="relative" x-data="{ open: @entangle('isOpen') }" @click.outside="open = false">
    ...
    >
        ...
        <div class="p-4 bg-slate-50 dark:bg-slate-950/80 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
            ...
            <span class="px-2.5 py-0.5 rounded-full text-xs font-mono font-bold {{ $totalAlertsCount > 0 ? 'bg-rose-500/20 text-rose-600' : 'bg-slate-200 dark:bg-slate-800 text-slate-500' }}">
        ...
        <div class="max-h-96 overflow-y-auto divide-y divide-slate-100 dark:divide-slate-800/60 p-2 space-y-1">
            ...
            <div class="p-3 rounded-2xl transition-colors {{ $notif['type'] === 'danger' ? 'bg-rose-50/60 dark:bg-rose-950/20 hover:bg-rose-50 dark:hover:bg-rose-950/40' : ($notif['type'] === 'warning' ? 'bg-amber-50/60 dark:bg-amber-950/20 hover:bg-amber-50 dark:hover:bg-amber-950/40' : 'bg-cyan-50/60 dark:bg-cyan-950/20 hover:bg-cyan-50 dark:hover:bg-cyan-950/40') }}">
                <div class="flex items-start gap-2.5">
                    <span class="text-xl shrink-0 mt-0.5">{{ $notif['icon'] }}</span>
                    <div class="flex-1 min-w-0">
                        <h4 class="text-xs font-black text-slate-900 dark:text-white font-tajawal">{{ $notif['title'] }}</h4>
                        <p class="text-[11px] text-slate-600 dark:text-slate-400 mt-0.5 leading-relaxed">{{ $notif['description'] }}</p>
                        @if(!empty($notif['link']))
                        <a 
                            href="{{ $notif['link'] }}" 

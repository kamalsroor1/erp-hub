<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-900/60 p-4 rounded-2xl border border-slate-800">
        ...
        <div class="flex items-center gap-2 flex-wrap">
            ...
            <button wire:click="openPaymentModal" class="px-4 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold rounded-xl shadow-lg shadow-emerald-600/30 flex items-center gap-1.5 transition-all">
                <span>💵 سداد دفعة وتنزيل مديونية</span>
            </button>
            @endif
            <a href="{{ route('suppliers.export.csv', $supplier->id) }}" class="px-3.5 py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-bold rounded-xl border border-slate-700 flex items-center gap-1.5 transition-all">
                📊 تصدير إكسيل
            </a>
            <button onclick="window.print()" class="px-3.5 py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-bold rounded-xl border border-slate-700 flex items-center gap-1.5 transition-all">
                🖨️ طباعة
            </button>
    ...
    <div class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
        <div class="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-md p-6 space-y-4 shadow-2xl">
            ...
            <div class="bg-slate-950/60 p-3 rounded-xl border border-slate-800 text-xs">
                ...
                <strong class="text-white text-sm block mt-0.5">{{ $supplier->name }}</strong>
            </div>
            ...
            <form wire:submit.prevent="savePayment" class="space-y-4">
                ...
                <div>
                    ...
                    <textarea wire:model="paymentNotes" rows="2" class="w-full bg-slate-950 border border-slate-700 rounded-xl p-2.5 text-xs text-white" placeholder="مثال: سداد باقي دفعة التوريد"></textarea>
                </div>

<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-4">
            ...
            <div class="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-3 shadow-sm">
                ...
                <div class="grid grid-cols-2 sm:grid-cols-4 gap-2">
                    ...
                    <button type="button" wire:click="setPresetTargetWeight('250.000')" class="py-2 rounded-xl border text-xs font-bold transition-all cursor-pointer {{ $target_weight_grams == '250.000' ? 'bg-amber-600 border-amber-500 text-white shadow-lg' : 'bg-slate-50 dark:bg-slate-950 border-slate-300 dark:border-slate-800 text-slate-600 dark:text-slate-400' }}">
                        ربع كيلو (250 جم)
                    </button>
                    <button type="button" wire:click="setPresetTargetWeight('500.000')" class="py-2 rounded-xl border text-xs font-bold transition-all cursor-pointer {{ $target_weight_grams == '500.000' ? 'bg-amber-600 border-amber-500 text-white shadow-lg' : 'bg-slate-50 dark:bg-slate-950 border-slate-300 dark:border-slate-800 text-slate-600 dark:text-slate-400' }}">
                        نصف كيلو (500 جم)
                    </button>
                    <button type="button" wire:click="setPresetTargetWeight('1000.000')" class="py-2 rounded-xl border text-xs font-bold transition-all cursor-pointer {{ $target_weight_grams == '1000.000' ? 'bg-amber-600 border-amber-500 text-white shadow-lg' : 'bg-slate-50 dark:bg-slate-950 border-slate-300 dark:border-slate-800 text-slate-600 dark:text-slate-400' }}">
                        كيلو كامل (1000 جم)
                    </button>
                </div>
            </div>
                    ...
                    <table class="w-full text-right text-xs">
                        ...
                        <tbody class="divide-y divide-slate-200 dark:divide-slate-800">
                            ...
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/20">
                                <td class="p-3 font-bold text-slate-800 dark:text-slate-200">{{ $comp['name'] }}</td>
                                <td class="p-3">
                                    <div class="flex items-center gap-1 justify-center">
                                        <input 
                                            ...
                                            class="w-16 bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 rounded-lg px-2 py-1 text-center font-mono font-bold text-amber-600 dark:text-amber-400 text-xs"
                                        >
                                ...
                                <td class="p-3 text-center">
                                    <button wire:click="removeComponent({{ $idx }})" class="text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 cursor-pointer">🗑️</button>
                                </td>
                            </tr>
                            @empty
                            <tr>
            ...
            <div class="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-2 shadow-sm">
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    <button 
                        ...
                        class="px-3 py-1.5 rounded-xl bg-slate-50 dark:bg-slate-950 hover:bg-slate-100 dark:hover:bg-slate-800 border border-slate-300 dark:border-slate-800 hover:border-amber-500/50 text-slate-800 dark:text-slate-200 text-xs font-semibold flex items-center gap-1.5 transition-all cursor-pointer"
                    >
                        <span>+ {{ $cof->name }}</span>
                    </button>
                    @endforeach
            ...
            <div class="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-3 shadow-sm">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="p-3 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 space-y-1.5">
                        <label class="block text-[11px] font-bold text-emerald-600 dark:text-emerald-400">حبهان (هيل) أخضر هندي:</label>
                        <div class="flex items-center gap-2">
                            <input 
                                type="number" 
                                step="0.5" 
                                ...
                                class="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-lg px-3 py-1.5 text-xs font-mono font-bold text-emerald-600 dark:text-emerald-400 text-center"
                            >
                            <span class="text-xs text-slate-500 dark:text-slate-400">جرام</span>
                        </div>
                    </div>

                    <div class="p-3 bg-slate-50 dark:bg-slate-950 rounded-xl border border-slate-200 dark:border-slate-800 space-y-1.5">
                        <label class="block text-[11px] font-bold text-teal-600 dark:text-teal-400">مستكة يوناني فصوص حرة:</label>
                        <div class="flex items-center gap-2">
                            <input 
                                type="number" 
                                step="0.5" 
                                ...
                                class="w-full bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-lg px-3 py-1.5 text-xs font-mono font-bold text-teal-600 dark:text-teal-400 text-center"
                            >
                            <span class="text-xs text-slate-500 dark:text-slate-400">جرام</span>
                        </div>
                    </div>
        ...
        <div class="space-y-4">
            ...
            <div class="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-3 shadow-sm">
                <div>
                    <label class="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">اسم التوليفة المخصوصة:</label>
                    <input type="text" wire:model="blend_name" class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 rounded-xl px-3 py-2 text-xs text-slate-900 dark:text-white">
                </div>
                ...
                <div class="grid grid-cols-2 gap-2">
                    <div>
                        <label class="block text-[11px] font-bold text-slate-500 dark:text-slate-400 mb-1">درجة التحميص:</label>
                        <select wire:model="roast_type" class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 rounded-lg px-2 py-1.5 text-xs text-slate-900 dark:text-white">
                    ...
                    <div>
                        <label class="block text-[11px] font-bold text-slate-500 dark:text-slate-400 mb-1">درجة الطحن:</label>
                        <select wire:model="grind_level" class="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 rounded-lg px-2 py-1.5 text-xs text-slate-900 dark:text-white">
            ...
            <div class="bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 space-y-3 shadow-sm">
                <h3 class="text-sm font-bold text-slate-900 dark:text-white border-b border-slate-200 dark:border-slate-800 pb-2">تكلفة وسعر بيع التوليفة</h3>

                <div class="space-y-2 text-xs">
                    <div class="flex justify-between text-slate-500 dark:text-slate-400">
                        <span>الوزن الكلي للتوليفة:</span>
                        <span class="font-mono font-bold text-amber-600 dark:text-amber-400">{{ number_format($total_weight_grams, 1) }} جم</span>
                    </div>
                    <div class="flex justify-between text-slate-500 dark:text-slate-400">
                        <span>تكلفة الخامات المخزنية:</span>
                        <span class="font-mono text-slate-700 dark:text-slate-300">{{ number_format($blended_cost_price, 2) }} ج.م</span>
                    </div>
                ...
                <button 
                    ...
                    class="w-full mt-4 py-3 bg-gradient-to-r from-amber-600 to-orange-500 hover:from-amber-500 hover:to-orange-400 text-white font-bold rounded-xl shadow-lg shadow-amber-600/30 text-xs transition-all active:scale-95 flex items-center justify-center gap-2 cursor-pointer"
                >

<div class="space-y-4">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-900/60 p-4 rounded-2xl border border-slate-800">
        <div>
            ...
            <p class="text-xs text-slate-400">تعديل الأصناف، الأوزان، الأسعار، أو طريقة السداد مع إعادة موازنة المخزون وحساب العميل تلقائياً</p>
        </div>
        <div class="flex items-center gap-2">
            <a href="{{ route('invoices.show', $invoice_id) }}" class="px-3 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-xl text-xs font-bold transition-all">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-4">
            ...
            <div class="bg-slate-900 p-4 rounded-2xl border border-slate-800 space-y-3">
                ...
                <div class="flex flex-wrap items-center gap-1.5 pt-1 text-[11px]">
                    <button wire:click="$set('selectedCategory', 'all')" class="px-3 py-1 rounded-xl font-bold transition-colors {{ $selectedCategory === 'all' ? 'bg-amber-600 text-white' : 'bg-slate-800/80 text-slate-300 hover:bg-slate-800' }}">
                        الكل
                    </button>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 gap-2.5 pt-2">
                    ...
                    <div class="p-2.5 rounded-xl bg-slate-950/80 border border-slate-800/80 hover:border-amber-500/50 transition-all flex flex-col justify-between group">
                        <div>
                            <div class="font-bold text-slate-200 text-xs line-clamp-1 group-hover:text-amber-400 transition-colors">{{ $prod->name }}</div>
                        ...
                        <div class="mt-2.5 pt-2 border-t border-slate-900 flex items-center gap-1">
                            @if($prod->unit === 'كجم')
                                ...
                                <button type="button" wire:click="addItem({{ $prod->id }}, '1.000')" title="كيلو كامل" class="flex-1 py-1 rounded bg-amber-600 hover:bg-amber-500 text-[9px] font-black text-slate-950 transition-colors">
                                    1كجم
                                </button>
                            @else
                                <button type="button" wire:click="addItem({{ $prod->id }}, '1.000')" class="w-full py-1 rounded bg-amber-500/20 hover:bg-amber-500 text-amber-300 hover:text-slate-950 text-[10px] font-bold transition-colors">
                                    + إضافة 1 {{ $prod->unit }}
                                </button>
                            @endif
                        </div>
            ...
            <div class="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden shadow-sm">
                <div class="p-3 bg-slate-950/60 border-b border-slate-800 flex items-center justify-between text-xs font-bold text-slate-300">
                    ...
                    <span class="text-[11px] text-amber-400 font-normal">تعديل مباشر على الأوزان والكميات والأسعار</span>
                </div>
                            ...
                            <tr class="hover:bg-slate-800/20">
                                    ...
                                    <div class="space-y-1.5">
                                        <div class="relative flex items-center">
                                            <input 
                                                ...
                                                class="w-full bg-slate-950 border border-slate-700 rounded-lg px-2 py-1 text-center text-xs font-mono font-bold text-amber-400 focus:outline-none focus:border-amber-500"
                                            >
                                        ...
                                        <div class="flex items-center gap-1 justify-center">
                                            ...
                                            <button type="button" wire:click="setLineWeightPreset({{ $idx }}, '1.000')" title="كيلو كامل" class="px-1.5 py-0.5 bg-amber-950 border border-amber-800 hover:bg-amber-800 text-[9px] rounded font-mono text-amber-300 font-bold">1kg</button>
                                        </div>
                                        @endif
                                    </div>
                                </td>
                                <td class="p-3">
                                    <input 
                                        ...
                                        class="w-full bg-slate-950 border border-slate-700 rounded-lg px-2 py-1 text-center text-xs font-mono text-white focus:outline-none focus:border-amber-500"
                                    >
                                </td>
                                <td class="p-3">
                                    <input 
        ...
        <div class="space-y-4">
            ...
            <div class="bg-slate-900 p-4 rounded-2xl border border-slate-800 space-y-3">
                ...
                <select wire:model.live="customer_id" class="w-full bg-slate-950 border border-slate-700 rounded-xl px-3 py-2.5 text-xs text-white focus:outline-none focus:border-amber-500">
                ...
                <div class="pt-2">
                    ...
                    <input 
                        ...
                        class="w-full bg-slate-950 border border-slate-700 rounded-xl px-3 py-2 text-xs font-mono font-bold text-white focus:outline-none focus:border-amber-500"
                    >
                </div>
                @endif
            </div>
            ...
            <div class="bg-slate-900 p-5 rounded-2xl border border-slate-800 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-2">
                    <div>
                        <label class="block text-[11px] font-semibold text-slate-400 mb-1">نوع خصم الفاتورة:</label>
                        <select wire:model.live="discount_type" class="w-full bg-slate-950 border border-slate-700 rounded-xl px-2 py-1.5 text-xs text-white">
                            <option value="fixed">مبلغ ثابت (ج.م)</option>
                            <option value="percentage">نسبة مئوية (%)</option>
                        </select>
                    </div>
                    <div>
                        <label class="block text-[11px] font-semibold text-slate-400 mb-1">قيمة الخصم:</label>
                        <input 
                            type="number" 
                            step="0.01" 
                            min="0" 
                            wire:model.live.debounce.250ms="discount_value" 
                            class="w-full bg-slate-950 border border-slate-700 rounded-xl px-2 py-1.5 text-xs font-mono text-rose-400 font-bold"
                        >
                    </div>
                </div>
                ...
                <div class="space-y-2 pt-2 border-t border-slate-800 text-xs">
                    <div class="flex items-center justify-between text-slate-400">
                        ...
                        <span class="font-mono text-white font-bold">{{ number_format($subtotal, 2) }} ج.م</span>
                    </div>
                    ...
                    <div class="flex items-center justify-between text-sm font-black text-amber-400 pt-2 border-t border-slate-800">
                    ...
                    <div class="flex items-center justify-between text-slate-300">
                        ...
                        <span class="font-mono font-bold {{ bccomp($remaining_amount, '0.000', 3) > 0 ? 'text-rose-400' : 'text-emerald-400' }}">
                ...
                <!-- Notes -->
                <div>
                    <label class="block text-[11px] font-semibold text-slate-400 mb-1">ملاحظات الفاتورة:</label>
                    <textarea wire:model="notes" rows="2" placeholder="ملاحظات العميل أو تفاصيل الطحن والتسليم..." class="w-full bg-slate-950 border border-slate-700 rounded-xl p-2 text-xs text-white focus:outline-none focus:border-amber-500"></textarea>
                </div>
                ...
                <div class="space-y-2 pt-2">
                    <button 
                        ...
                        class="w-full py-3 bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-slate-950 font-black rounded-xl text-sm shadow-lg shadow-amber-500/20 flex items-center justify-center gap-2 transition-all transform active:scale-95 cursor-pointer"
                    >

...
* **Zero Layout Shift Sidebar:** إضافة كلاسات عرض CSS ثابتة للقائمة الجانبية وتهيئتها في `<head>` فوراً قبل معالجة الصفحة مع تفعيل الانتقال السلس `transition` فقط عند النقر اليدوي من المستخدم لمنع وميض وتمدد القائمة عند كل انتقال بين الصفحات.

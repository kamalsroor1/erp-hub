        ...
        <div class="space-y-6 font-tajawal">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <div class="bg-slate-900 border border-slate-800 rounded-3xl p-4 space-y-1">
                    <span class="text-[11px] font-bold text-slate-400">{{ $t('activity.today_total') }}</span>
                    <div class="text-xl font-black font-mono text-white">{{ stats.today_total || 0 }}</div>
                </div>

                <div class="bg-slate-900 border border-slate-800 rounded-3xl p-4 space-y-1">
                    <span class="text-[11px] font-bold text-slate-400">{{ $t('activity.today_critical') }}</span>
                    <div class="text-xl font-black font-mono text-rose-400">{{ stats.today_critical || 0 }}</div>
                </div>

                <div class="bg-slate-900 border border-slate-800 rounded-3xl p-4 space-y-1">
                    <span class="text-[11px] font-bold text-slate-400">{{ $t('activity.today_users') }}</span>
                    <div class="text-xl font-black font-mono text-emerald-400">{{ stats.today_users || 0 }}</div>
                </div>

                <div class="bg-slate-900 border border-slate-800 rounded-3xl p-4 space-y-1">
                    <span class="text-[11px] font-bold text-slate-400">{{ $t('activity.today_stores') }}</span>
            ...
            <div class="flex flex-col sm:flex-row items-center justify-between gap-3 bg-slate-900/60 border border-slate-800 p-2.5 rounded-2xl">
                ...
                <div class="flex flex-wrap items-center gap-1.5 text-xs">
                    <span class="text-slate-400 font-bold text-[11px] px-2">{{ $t('activity.period') }}</span>
                    <button
                        v-for="p in [
            ...
            >
                <div class="space-y-4">
                    <div class="space-y-1.5">
                        <label class="text-xs font-bold text-slate-300">{{ $t('activity.search_filter_desc') }}</label>
                        <input
                    ...
                    <div class="space-y-1.5">
                        <label class="text-xs font-bold text-slate-300">{{ $t('activity.module_filter') }}</label>
                        <select
                    ...
                    <div class="space-y-1.5">
                        <label class="text-xs font-bold text-slate-300">{{ $t('activity.action_filter') }}</label>
                        <select
                    ...
                    <div class="space-y-1.5">
                        <label class="text-xs font-bold text-slate-300">{{ $t('activity.user_filter') }}</label>
                        <select
                    ...
                    <div class="space-y-1.5">
                        <label class="text-xs font-bold text-slate-300">{{ $t('activity.store_filter') }}</label>
                        <select
                ...
                <div class="bg-slate-900 border border-slate-800 rounded-3xl max-w-xl w-full p-6 space-y-4 shadow-2xl">
                    ...
                    <div class="space-y-3 text-xs">
                        <div class="grid grid-cols-2 gap-2 bg-slate-950 p-3 rounded-2xl border border-slate-800">
                            <div><span class="text-slate-400">{{ $t('common.user') }}:</span> <strong class="text-white">{{ selectedLog.user_name }}</strong></div>
                            <div><span class="text-slate-400">{{ $t('common.store') }}:</span> <strong class="text-white">{{ selectedLog.store_name }}</strong></div>
                            <div><span class="text-slate-400">{{ $t('common.date') }}:</span> <span class="text-slate-300 font-mono">{{ selectedLog.created_at }}</span></div>
                            <div><span class="text-slate-400">{{ $t('activity.ip_address') }}:</span> <span class="text-slate-300 font-mono">{{ selectedLog.ip_address || '-' }}</span></div>
                        </div>

                        <div>
                            <span class="text-slate-400 font-bold block mb-1">{{ $t('activity.full_description') }}</span>
                        ...
                        <div v-if="selectedLog.user_agent">
                            <span class="text-slate-400 font-bold block mb-1">{{ $t('activity.user_agent') }}</span>
                        ...
                        <div v-if="selectedLog.payload">
                            <span class="text-slate-400 font-bold block mb-1">{{ $t('activity.payload_data') }}</span>

        ...
        <div class="space-y-6 font-tajawal">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p v-if="active_shift" class="text-xs text-slate-400 mt-0.5">
                            {{ $t('treasury.cashier_label') }}: <strong class="text-white">{{ active_shift.user_name }}</strong> | {{ $t('treasury.opened_at_label') }}: <span class="font-mono text-amber-400">{{ active_shift.opened_at }}</span>
                        </p>
                    </div>
                </div>

                <div v-if="active_shift" class="text-left font-mono">
                    <span class="text-xs text-slate-400 font-tajawal">{{ $t('treasury.opening_cash') }}</span>
                    <div class="text-lg font-black text-emerald-400">
                        {{ formatMoney(active_shift.opening_cash_balance) }} <span class="text-xs text-white">{{ $t('common.currency') }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div class="bg-slate-900 border border-slate-800 rounded-3xl p-5 shadow-sm space-y-2">
                    <span class="text-xs text-emerald-400 font-bold">{{ $t('treasury.inflow_cash') }}</span>
                    <div class="text-2xl font-black font-mono text-emerald-400">
                        {{ formatMoney(summary.total_cash_in) }} <span class="text-xs text-white">{{ $t('common.currency') }}</span>
                    </div>
                    <div class="text-[11px] text-slate-400 font-bold space-y-0.5 pt-1 border-t border-slate-800/80">
                        <div>{{ $t('treasury.cash_sales') }}: <span class="font-mono text-white">{{ formatMoney(summary.cash_sales) }}</span></div>
                        <div>{{ $t('treasury.customer_collections') }}: <span class="font-mono text-white">{{ formatMoney(summary.customer_payments) }}</span></div>
                    </div>
                </div>

                <!-- Total Outflow -->
                <div class="bg-slate-900 border border-slate-800 rounded-3xl p-5 shadow-sm space-y-2">
                    <span class="text-xs text-rose-400 font-bold">{{ $t('treasury.outflow_cash') }}</span>
                    <div class="text-2xl font-black font-mono text-rose-400">
                        {{ formatMoney(summary.total_cash_out) }} <span class="text-xs text-white">{{ $t('common.currency') }}</span>
                    </div>
                    <div class="text-[11px] text-slate-400 font-bold space-y-0.5 pt-1 border-t border-slate-800/80">
                        <div>{{ $t('treasury.supplier_payments') }}: <span class="font-mono text-white">{{ formatMoney(summary.supplier_payments) }}</span></div>
                        <div>{{ $t('treasury.operating_expenses') }}: <span class="font-mono text-white">{{ formatMoney(summary.total_expenses) }}</span></div>
                    </div>
                </div>

                <!-- Net Day Cash -->
                <div class="bg-slate-900 border border-slate-800 rounded-3xl p-5 shadow-sm space-y-2">
                    <span class="text-xs text-slate-400 font-bold">{{ $t('treasury.net_cash_today') }}</span>
                    <div
                        class="text-2xl font-black font-mono"
                        :class="summary.net_cash_today >= 0 ? 'text-amber-400' : 'text-rose-400'"
                    >
                        {{ formatMoney(summary.net_cash_today) }} <span class="text-xs text-white">{{ $t('common.currency') }}</span>
                    </div>
                    <div class="text-[11px] text-slate-400 font-bold pt-1 border-t border-slate-800/80">
                        {{ $t('treasury.recorded_credit_sales') }}: <span class="font-mono text-amber-300">{{ formatMoney(summary.credit_sales) }} {{ $t('common.currency') }}</span>
                    </div>
                </div>
                ...
                <div class="bg-slate-900 border border-amber-500/30 rounded-3xl p-5 shadow-sm space-y-2 bg-gradient-to-br from-slate-900 to-amber-950/20">
                    <span class="text-xs text-amber-400 font-black">{{ $t('treasury.expected_in_drawer_now') }}</span>
                    <div class="text-3xl font-black font-mono text-amber-400">
                        {{ formatMoney(summary.expected_cash_in_drawer) }} <span class="text-xs text-white">{{ $t('common.currency') }}</span>
                    </div>
                    <div class="text-[11px] text-slate-400 font-bold pt-1 border-t border-slate-800/80">
                        {{ $t('treasury.including_opening_cash') }}: <span class="font-mono text-white">{{ formatMoney(summary.opening_cash_balance) }}</span>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div class="bg-slate-900 border border-slate-800 rounded-3xl p-5 shadow-sm space-y-4">
                    <div class="flex items-center justify-between border-b border-slate-800 pb-3">
                        <div class="flex items-center gap-2">
                            <span>🧾</span>
                            <h3 class="font-black text-sm text-white">{{ $t('treasury.today_invoices') }} ({{ invoices.length }})</h3>
                        </div>
                    </div>
                ...
                <div class="bg-slate-900 border border-slate-800 rounded-3xl p-5 shadow-sm space-y-4">
                    <div class="flex items-center justify-between border-b border-slate-800 pb-3">
                        <div class="flex items-center gap-2">
                            <span>💸</span>
                            <h3 class="font-black text-sm text-white">{{ $t('treasury.today_expenses') }} ({{ expenses.length }})</h3>
                        </div>
                    </div>
                        ...
                        <table class="w-full text-right text-xs">
                            ...
                            <tbody class="divide-y divide-slate-800/60 font-sans">
                                <tr v-for="exp in expenses" :key="exp.id" class="hover:bg-slate-800/30 transition">
                                    ...
                                    <td class="py-2.5 font-mono font-bold text-rose-400">{{ formatMoney(exp.amount) }} {{ $t('common.currency') }}</td>

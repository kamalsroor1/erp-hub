    ...
    <div class="min-h-screen bg-slate-950 text-slate-100 flex flex-col antialiased selection:bg-indigo-500 selection:text-white font-tajawal" dir="rtl">
        ...
        <header class="h-16 border-b border-indigo-900/40 bg-slate-900/90 backdrop-blur-md sticky top-0 z-40 px-4 flex items-center justify-between">
            ...
            <div class="flex items-center gap-2 sm:gap-3">
                <Link
                    href="/"
                    class="h-10 px-3.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-xs font-bold text-slate-300 flex items-center gap-1.5 transition active:scale-95 border border-slate-700 shadow-xs"
                >
                    <span>☕</span>
            ...
            >
                ...
                <div class="mt-auto p-4 rounded-2xl bg-gradient-to-br from-indigo-950/60 to-slate-900 border border-indigo-800/40 text-xs space-y-2">
                    <div class="font-black text-white flex items-center gap-1.5 text-xs">
                        <span>⚡</span>
                        <span>Multi-Database Engine</span>
                    </div>
                    <p class="text-[11px] text-slate-400 leading-relaxed">
                        Instant schema provisioning per tenant workspace.
                    </p>
                </div>
            </aside>
        </div>
        ...
        <div class="flex-1 flex overflow-hidden">
            ...
            <aside class="hidden md:flex w-64 bg-slate-900/60 border-l border-indigo-900/30 flex-col p-4 space-y-6 shrink-0">
                <div class="space-y-1">
            ...
            <main class="flex-1 overflow-y-auto p-3.5 sm:p-6 lg:p-8 space-y-6">
                ...
                <div v-if="flash?.success" class="p-4 rounded-2xl bg-emerald-500/15 border border-emerald-500/30 text-emerald-300 font-bold text-xs flex items-center justify-between shadow-xs">
                    <span>✓ {{ flash.success }}</span>
                </div>
                <div v-if="flash?.error" class="p-4 rounded-2xl bg-rose-500/15 border border-rose-500/30 text-rose-300 font-bold text-xs flex items-center justify-between shadow-xs">
